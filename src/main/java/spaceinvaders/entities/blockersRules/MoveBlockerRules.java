...
public class MoveBlockerRules extends MoveBlockerRulesApplierDefaultImpl {
	 ...
	 * @param ennemy The enemy witch is hit
	 * @throws IllegalMoveException
	 */
	public void moveBlockerRule(Laser laser, Enemy enemy, Point position) throws IllegalMoveException {
	...
	public void moveBlockerRule(Laser laser, SmallAlien alien) throws IllegalMoveException {
		moveBlockerRule(laser, (Enemy) alien, alien.getLocationInArray());
	}
	 ...
	 */
	protected void kill(Laser laser, Enemy enemy, Point positionInArray) {
		enemy.getArray().increaseSpeed();
		enemy.getArray().removeEnemyFromPosition(positionInArray);
		gameData.getUniverse().removeGameEntity(enemy);
		gameData.getUniverse().removeGameEntity(laser);
		laser.getShooter().resetShoot();
		gameData.getScore().setValue(gameData.getScore().getValue() + enemy.score());
	}
}
